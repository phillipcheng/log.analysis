...
public class InvokeNullableReducer extends Reducer<Text, Text, Text, NullWritable>{
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for (String[] ret: rets){
			if (ETLCmd.SINGLE_TABLE.equals(ret[2])){
				context.write(new Text(ret[0]), NullWritable.get());
			}else{
				mos.write(new Text(ret[0]), NullWritable.get(), ret[2]);
			}
		}
	}
}
