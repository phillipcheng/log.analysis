...
public abstract class SchemaCmd extends ACmd{
	...
	private void safeWriteSchemaFile(String defaultFs, String path, boolean directory, LogicSchema schema) throws Exception {
	...
	private LogicSchema createTableSchema(LogicSchema tableSchema, String schemaFile, String name, List<String> attrIds,
		...
		if (dbtype != DBType.NONE){
			int result = DBUtil.executeSqls(Arrays.asList(new String[] {createTableSql}), pc());
			//gen report info
	...
	private LogicSchema updateTableSchema(LogicSchema tableSchema, String schemaFile, String name, List<String> attrIds, List<String> attrNames,
		...
		if (existAttrs.containsAll(attrNames)) {//
			...
			//execute the sql
			if (dbtype != DBType.NONE){
				int result = DBUtil.executeSqls(updateTableSqls, pc());
				//gen report info
	...
	private void updateSchemaIndex(LogicSchema logicSchema, String schemaFile) throws Exception{
	...
	public List<String> updateSchema(Map<String, List<String>> attrNamesMap, Map<String, List<FieldType>> attrTypesMap){
		...
		for (String sql: loginfo) {
			...
			HdfsUtil.appendDfsFile(fs(), this.createTablesSqlFileName, Arrays.asList(sql));
		}
